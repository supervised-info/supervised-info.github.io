...
Stand 2026-09-05. https://github.com/supervised-info/einkauf-watch — iPhone + Apple Watch (**Build 62**). Die App hat zwei Reiter **Einkauf | To-Do**. Daten, Persistenz und Backups sind **strikt getrennt**: Einkauf bleibt `kind: "einkauf-backup"` / `einkauf-local.json`; To-Do ist `format: "todo-v3-json"` / `todo-local.json`. Kein gemeinsames Store, kein Live-localStorage-Sync, die App scrapt die PWA nicht. HTML-To-Do bleibt die eigenständige PWA `todo/` (Keys `todo-v3*`).

Dieses Delta **bleibt bewusst** und darf von Regeneratoren **nicht geschlossen** werden:

- **Nur HTML / in dieser Spec behalten:** Markdown kopieren / Datei exportieren / teilen; Import `.md`/`.txt`; **nach Bring exportieren**; **nach Erinnerungen exportieren**; extra Läden-JSON `kind: "einkauf-laeden"`; `h1` „Einkaufsliste“ am Site-Mast.
- **Nur native / nicht ins HTML:** Tab **Einkauf | To-Do**; Watch; **Siri / App Intents** (kein In-App-Mikro, kein `Speech.framework`); Watch-Complication (Einkauf + To-Do); iPhone-Homescreen-Widget (nur Einkauf); **PDF Liste teilen** (beide Tabs); Erscheinungsbild **System** (folgt iPhone-Appearance) plus Hell/Dunkel und Creme/Blau **nur in den Einstellungen**, nicht in der engen Toolbar; iPhone-Nav ohne große Titel „Einkaufsliste“ / „To-Do“, Toolbar kompakt (Build 62); **iCloud-Inbox** nur Einkauf (Build 59–61). HTML behält die Site-Mast-Buttons Theme + Palette (`theme-btn`, `#paletteBtn`) und die Shared Keys `supervised-info.theme` / `supervised-info.palette`.
...
- **To-Do (nativer zweiter Tab, nicht nachbauen):** Bridge `todo-v3-json` (Roundtrip mit der HTML-PWA `todo/`). Phase 8 MD/CSV (Build 53) und Phase 10 benannte Listen (Build 55) sind nativ geliefert — HTML-To-Do behält MD/CSV. Overflow **Liste teilen** als PDF. Watch nur Geh-Modus + Complication Label **To Do** (Leerzeichen; offene Anzahl, bei 0 **erledigt**). Siri **Todo** (ein Token, nicht „To Do“) — siehe Sprache. Wieder öffnen, Sort und Suche nur auf dem iPhone; Watch bleibt Geh-Modus. Build 56–58: JSON-Backup zusätzlich unter **Einstellungen → To-Do Backup**; Import-`revision`-Floor analog Einkauf (HTML hat keine `revision`); iPhone-Zeile `#uid` Badge + reopen-Pills + **Abgeschlossen-Datum** wie HTML. iCloud-Inbox nie für To-Do.
- **iCloud-Inbox (nur Einkauf, native only, Build 59–61):** Overflow **Inbox verbinden…** (Dateien-Picker → Security-scoped Bookmark auf `Einkauf-Inbox/inbox.txt`) und **Inbox abrufen** (Auswahl-Sheet, alle markiert; **Übernehmen** → `addItems(fromSpeech:)`; Abgewählte bleiben in der Datei; **Löschen** pro Zeile ohne Import). Zweit-iPhone: Kurzbefehle **Einkauf-Inbox eintragen** / **Einkauf-Inbox vorlesen** (iOS 26, **An Textdatei anhängen**). Kein Server, kein CloudKit, kein iCloud-Entitlement. Phase 4 (concurrent Append) ist offen — nicht ins HTML.
- **iPhone-Nav (Build 62):** kein großer Navigationstitel „Einkaufsliste“ / „To-Do“ (leer, `.inline`); Toolbar kompakt. Der Tab-Name reicht. HTML behält die `h1`-Titel.
...
- Bewusstes Delta zur nativen App (siehe oben) nicht angleichen — kein Siri, kein Watch-Mikro/`Speech.framework`, kein PDF, keine Complication, kein To-Do-Tab, keine iCloud-Inbox, keine kompakte Native-Nav ohne `h1` im HTML.
...
- [ ] Native-Delta bleibt (kein Watch, kein Siri/`Speech.framework`, kein To-Do-Tab, kein PDF-Teilen, keine Complication, kein System-Theme in der Toolbar, keine iCloud-Inbox, kein Live-Sync — nur Backup-Datei; HTML-To-Do bleibt `todo/`).
