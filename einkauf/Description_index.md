...
Stand der Live-PWA: **2026-09-06**. Diese Spec beschreibt die Seite so, dass sie daraus neu erzeugt werden kann.
...
- `einkauf/sw.js` — Cache-Name **aktuell** `einkauf-offline-v25`
...
**Bei jedem Deploy Cache-Namen hochzählen** (`v25` → `v26` …), sonst bleiben alte Assets. Nie denselben Cache-Namen wiederverwenden.
...
`id` = `i` + time36 + random. `guessDept(name)` in dieser Reihenfolge (wie Native): **zuerst** `mappings[mappingKey(name)]`, falls bekannte `DEPTS`-ID; dann Sonderregeln (TK, Eistee, Schorle, Chips, Eis); dann längstes Keyword aus `DICT_SRC`; sonst `sonstiges`. Select **und** Cross-Dept-Drop schreiben denselben Mapping-Key — Nutzerkorrekturen gewinnen vor Wörterbuch und Sonderregeln. Sort in Dept: `ord`, dann `added`, dann `localeCompare` de. **Keine** Umsortierung nach `urgency` — nur Chip, gleiche Reihenfolge wie Native Build 63.

Zwei unabhängige optionale Felder **nur Einkauf** (nicht To-Do, nicht Inbox):

- **`imported`** (`boolean`): fehlender Key = `false`. **Setzen** (`true`) nur beim Fremd-Datei-Import **Daten importieren** (`.md`/`.txt` via `parseMarkdown`). **Nie** Tippen/`addItem`, Stamm/`applyStaple` / `applyAllStaples`, gespeicherte Listen/`applySavedList`. Backup-**Restore** (`applyBackupImport` / `sanitizeItems`) stempelt nicht nach — Werte bleiben wie im JSON (fehlend = false). Export schreibt das Feld **wie gespeichert** (kein Nachstempeln aller Artikel auf eigenem Re-Export). In der UI nur Anzeige, **rechts** in `.row-trail` vor dem Urgency-Chip: teal Markierung `.imported-mark` mit Token `--slate` (**kein** Grün/`--good`, kein Toggle, **kein linker Streifen**). Native setzt `imported` nur beim Inbox-Abruf und zeigt den Streifen links; HTML hat keine Inbox — MD/txt ist der Fremd-Datei-Pfad — und setzt die Markierung rechts neben den Chip.
- **`urgency`**: `"urgent"` | `"normal"` | `"later"`; fehlender oder unbekannter Key = `"normal"`. Nutzer wechselt per Chip (Geh-Modus und Bearbeiten): `urgent` → `normal` → `later` → `urgent`. Icons: eilig ⚡, normal **leerer umrandeter Chip** (nur Rahmen `--rule`, keine Glyphe, kein ○, kein –, kein Leerzeichen-Inhalt, feste Min-Breite `.urgency-chip`), später ↓ (U+2193). Encode schreibt das Feld immer. HTML-Normal ist bewusst umrandet (Nutzer final); Native bleibt `Color.clear` ohne Rahmen.

Alte `einkauf_v1`-Stände und Backups ohne die Keys laden weiter. Beide Felder kombinierbar.

UI je Item, links: Checkbox + Name (Bearbeiten zusätzlich Grip). Rechts trailing in `.row-trail`: (1) teal Import-Markierung read-only, (2) Dringlichkeits-Chip. Dann Dept-Select, Löschen `×`. Name: Klick rename; Enter speichern, Escape abbrechen. Empty: „Noch nichts auf der Liste.“
...
Geh-Modus: `body.walk` — Button-Label „Bearbeiten“, `aria-pressed` true; kein Grip/Select/Delete, Name nicht editierbar (CSS + Render). Links nur Checkbox + Name; rechts Import-Markierung + Chip (Chip tippbar). Persistiert.
...
Save schreibt **kein** `palette` ins einkauf_v1-Objekt (nur theme + Rest). Palette nur Shared Key. Alte Speicherstände ohne `savedLists`: `[]`. Alte Stände ohne `hideDone`: `false`. Alte Items ohne `imported`/`urgency`: `false` / `"normal"`.
...
- Import: Überschriften → Dept; Checkbox, Bullet oder nummerierte Liste; führende `NN ` (Bring-Nummern) strippen. Confirm ersetzen vs. anhängen wenn Liste nicht leer. Neu angelegte Artikel aus MD/txt: `imported: true`, `urgency: "normal"`. JSON-Backup- oder Läden-Datei wird in demselben Import-Pfad erkannt (`tryImportStoresText`) und **stempelt nicht** — Backup geht durch `applyBackupImport`.
...
Share oder Download `einkauf-backup.json`. Import Confirm ersetzt Liste und Läden. Alte Backups ohne `savedLists`: leeres Array. Stamm und eingebaute Läden bleiben unangetastet (Seeds mergen). **`hideDone` gehört nicht ins Backup** — Export lässt es weg, Import ändert das lokale Flag nicht. `imported` und `urgency` werden mitgeschrieben (fehlend beim Lesen = false / `normal`); Restore stempelt nicht nach. Roundtrip behält beide Werte.

Escape schließt Menüs/Sheet. Enter in neuen Laden/Stamm legt an.

## Native Begleit-App (nur dokumentieren, nicht nachbauen)

Stand 2026-09-06. https://github.com/supervised-info/einkauf-watch — iPhone + Apple Watch (**Build 63**). Die App hat zwei Reiter **Einkauf | To-Do**. Daten, Persistenz und Backups sind **strikt getrennt**:

- Einkauf: `kind: "einkauf-backup"` / `einkauf-local.json`
- To-Do: `format: "todo-v3-json"` / `todo-local.json`

Kein gemeinsames Store, kein Live-localStorage-Sync, die App scrapt die PWA nicht. HTML-To-Do bleibt die eigenständige PWA `todo/` (Keys `todo-v3*`). Benannte To-Do-Listen (`lists` / `listId`) sind **nativ und HTML** geliefert — kein Follow-up mehr.

Dieses Delta **bleibt bewusst** und darf von Regeneratoren **nicht geschlossen** werden:

- **Nur HTML / in dieser Spec behalten:** Markdown kopieren / Datei exportieren / teilen; Import `.md`/`.txt`; **nach Bring exportieren**; **nach Erinnerungen exportieren**; extra Läden-JSON `kind: "einkauf-laeden"`; `h1` „Einkaufsliste“ am Site-Mast; Site-Mast Theme + Palette (`theme-btn`, `#paletteBtn`) und Shared Keys `supervised-info.theme` / `supervised-info.palette`; Urgency-`normal` als **leerer umrandeter Chip** (Native: unsichtbar/`Color.clear`); Import-Markierung **rechts** vor dem Chip (Native: Streifen links).
...
- **iCloud-Inbox (nur Einkauf, native only, Build 59–61):** Overflow **Inbox verbinden…** (Dateien-Picker → Security-scoped Bookmark auf `Einkauf-Inbox/inbox.txt`) und **Inbox abrufen** (Auswahl-Sheet, alle markiert; **Übernehmen** → `addItems(fromSpeech:)`; Abgewählte bleiben in der Datei; **Löschen** pro Zeile ohne Import). Zweit-iPhone: Kurzbefehle **Einkauf-Inbox eintragen** / **Einkauf-Inbox vorlesen** (iOS 26, **An Textdatei anhängen**). Kein Server, kein CloudKit, kein iCloud-Entitlement. Phase 4 (concurrent Append, ungültiger Bookmark) ist offen — nicht ins HTML.
- **iPhone-Nav (Build 62):** kein großer Navigationstitel „Einkaufsliste“ / „To-Do“ (leer, `.inline`); Toolbar kompakt (`einkaufToolbarChrome`). Der Tab-Name reicht. Native-Umschalter **Edit** / **Geh-Modus** (To-Do: **Edit** / **Fertig**). HTML behält die `h1`-Titel und „Bearbeiten“.
...
- **Swipe-Löschen (beide Tabs, native only):** nur im **Edit**-Modus. Geh-Modus / To-Do-Listen-Modus: kein Swipe-Delete (`.deleteDisabled`, kein `.onDelete`). HTML behält den sichtbaren `×`-Knopf in der Artikelzeile.
- **Complication vs. App-Titel:** Einkauf-Complication zeigt nur die offene Anzahl, bei 0 das Wort **erledigt** (nicht `oo/xx/yy`). To-Do-Complication dasselbe Muster für die **aktuelle Liste**, Label **To Do**. Watch-Titel, iPhone-Widget, PDF-Meta und HTML-`#count` bleiben `oo/xx/yy`.
...
Gemeinsame Slice (HTML und Native, Stand 2026-09-06): Zähler `oo/xx/yy` (offen/erledigt/gesamt, leer `0/0/0`); `savedLists` füllen statt ersetzen; Sonstiges-Slot im Ladenweg; Einstellungen-Reihenfolge Aktueller Laden → Neuer Laden → Ladenweg → Stamm → Gespeicherte Listen → Wörterbuch; Wörterbuch aus dem lokalen Keyword-`DICT_SRC` plus **Meine Zuordnungen** aus dem bestehenden `mappings`-Objekt (`einkauf_v1` / Backup, kein neues Feld); `guessDept` zuerst Mapping, dann Sonderregeln, dann längstes Keyword; Auge blendet Erledigte aus (Flag geräte-lokal, nicht im Backup). HTML filtert Geh-Modus **und** Bearbeiten; iPhone-Edit bleibt ungefiltert. Einkauf-Artikel optional `imported` + `urgency` (Backup-Roundtrip, fehlend = false / `normal`; Chip ⚡ / umrandet-leer / ↓; teal `--slate`-Markierung; kein Sort nach Dringlichkeit). HTML: Markierung rechts vor dem Chip; Native: Streifen links. Native setzt `imported` nur Inbox; HTML nur MD/txt-Datenimport. To-Do-Brücke `todo-v3-json` teilt das Task-Shape inkl. `lists`/`listId` mit der HTML-PWA `todo/` — nicht mit `einkauf-backup`. Benannte Listen sind auf beiden Seiten geliefert.
...
- Bewusstes Delta zur nativen App (siehe oben) nicht angleichen — kein Siri, kein Watch-Mikro/`Speech.framework`, kein PDF, keine Complication, kein To-Do-Tab, keine iCloud-Inbox, keine kompakte Native-Nav ohne `h1`, kein Toolbar-Label **Edit** im HTML.

## Akzeptanzkriterien

- [ ] Offline nach erstem Besuch (SW v-Bump, aktuell v25).
...
- [ ] `imported` nur MD/txt-Datenimport (nie Tippen/Stamm/Saved-List; Backup-Restore und eigener Re-Export stempeln nicht); UI-Markierung teal/`--slate` **rechts** vor dem Chip, read-only, kein linker Streifen. `urgency` Chip ⚡ / umrandet-leer / ↓ (kein ○, kein –), Zyklus eilig→normal→später, Geh und Bearbeiten. Kein Sort nach Urgency. Nicht auf To-Do.
