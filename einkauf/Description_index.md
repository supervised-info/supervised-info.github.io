...
- `einkauf/sw.js` — Cache-Name **aktuell** `einkauf-offline-v24`
...
**Bei jedem Deploy Cache-Namen hochzählen** (`v24` → `v25` …), sonst bleiben alte Assets. Nie denselben Cache-Namen wiederverwenden.
...
UI je Item: optionaler Import-Streifen (nur Anzeige), Checkbox, Name (Klick rename; Enter speichern, Escape abbrechen), Dringlichkeits-Chip (tippbar, Zyklus wie oben), Dept-Select, Drag-Handle, Löschen `×`. Empty: „Noch nichts auf der Liste.“
...
Geh-Modus: `body.walk` — Button-Label „Bearbeiten“, `aria-pressed` true; kein Grip/Select/Delete, Name nicht editierbar (CSS + Render). Import-Streifen und Dringlichkeits-Chip bleiben sichtbar; Chip tippbar. Persistiert.
...
- **Nur HTML / in dieser Spec behalten:** Markdown kopieren / Datei exportieren / teilen; Import `.md`/`.txt`; **nach Bring exportieren**; **nach Erinnerungen exportieren**; extra Läden-JSON `kind: "einkauf-laeden"`; `h1` „Einkaufsliste“ am Site-Mast; Site-Mast Theme + Palette (`theme-btn`, `#paletteBtn`) und Shared Keys `supervised-info.theme` / `supervised-info.palette`; Urgency-`normal` als **leerer umrandeter Chip** (Native: unsichtbar/`Color.clear`).
...
- [ ] Offline nach erstem Besuch (SW v-Bump, aktuell v24).
...
- [ ] `imported` nur MD/txt-Datenimport (nie Tippen/Stamm/Saved-List; Backup-Restore und eigener Re-Export stempeln nicht); UI-Streifen teal/`--slate`, read-only. `urgency` Chip ⚡ / umrandet-leer / ↓ (kein ○, kein –), Zyklus eilig→normal→später, Geh und Bearbeiten. Kein Sort nach Urgency. Nicht auf To-Do.
