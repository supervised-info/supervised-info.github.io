...
- `einkauf/sw.js` — Cache-Name **aktuell** `einkauf-offline-v26`
...
**Bei jedem Deploy Cache-Namen hochzählen** (`v26` → `v27` …), sonst bleiben alte Assets. Nie denselben Cache-Namen wiederverwenden.
...
UI je Item, L→R: Checkbox + Name, dann Dringlichkeits-Chip, dann teal Import-Markierung **ganz rechts** (read-only). Bearbeiten zusätzlich Grip, Dept-Select, Löschen `×` (Markierung bleibt das letzte Element). Name: Klick rename; Enter speichern, Escape abbrechen. Empty: „Noch nichts auf der Liste.“
...
Geh-Modus: `body.walk` — Button-Label „Bearbeiten“, `aria-pressed` true; kein Grip/Select/Delete, Name nicht editierbar (CSS + Render). L→R: Checkbox + Name | Chip (tippbar) | Import-Markierung ganz rechts. Persistiert.
...
- [ ] Offline nach erstem Besuch (SW v-Bump, aktuell v26).
